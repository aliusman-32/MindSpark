...
def _ensure_chapters_from_lesson(lesson: VisualLesson):
    """Extract and normalize chapters from a lesson, with fallback logic."""
    chapters = []
    ...
    # Fallback: generate chapters from lesson text
...
def _build_quiz_payload_from_lesson(lesson: VisualLesson):
    """Generate quiz payload from lesson data using QuizAgent."""
    chapters = _ensure_chapters_from_lesson(lesson)
    quiz_agent = QuizAgent(data={"chapters": chapters})
    quiz_questions = quiz_agent.build_quiz_from_chapters(chapters)
...
async def generate_quiz(request: QuizGenerateRequest, db: Session = Depends(get_db)):
    """Generate a quiz for a lesson based on its chapters."""
...
async def list_quizzes(lesson_id: int, db: Session = Depends(get_db)):
    ...
    quizzes = (
        db.query(LessonQuiz)
        .filter(LessonQuiz.lesson_id == lesson_id)
        .order_by(LessonQuiz.created_at.desc())
        .all()
    )
...
async def list_quiz_attempts(user_id: Optional[int] = None, lesson_id: Optional[int] = None, db: Session = Depends(get_db)):
    """Retrieve quiz attempt history for a user."""
