...
class VideoGenerator:
    ...
    def _call_colab_api(self, prompt, retries=3):
        """Call the Stable Diffusion API running on Colab."""
        payload = {
            "prompt": prompt,
    ...
    def _create_title_slide(self, title, filename):
        """Create a title slide locally (unchanged)."""
    ...
    def _build_prompt(self, point, chapter_title=None):
        ...
        """
        # Base prompt with the key point
        base = f"{point}"

        # Style description – emphasize simplicity, no text, bright colors, child-friendly
        style = (
        ...
        # Optionally add chapter context
    ...
    def create_video(self, fps=24):
        ...
        for i in range(len(self.pause_positions_ms)):
            if i < len(self.pause_positions_ms) - 1:
                ...
                duration_ms = len(audio) - self.pause_positions_ms[i]
        ...
        for chapter_index, chapter in enumerate(self.chapters):
            ...
            frame_duration = chapter_durations[chapter_index] / num_frames
